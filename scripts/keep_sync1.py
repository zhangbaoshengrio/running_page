...
def parse_raw_data_to_nametuple(run_data, old_gpx_ids, with_download_tcx=False):
    ...
    if run_data.get("vendor", {}).get("source", "") == "Keep" and run_data.get(
        ...
        if with_download_tcx:
            if str(keep_id) not in old_gpx_ids:
                tcx_data = parse_points_to_tcx(run_points_data_gpx, start_time)
                download_keep_tcx(tcx_data, str(keep_id))
    heart_rate = None
...
def get_all_keep_tracks(email, password, old_tracks_ids, with_download_tcx=False):
    if with_download_tcx and not os.path.exists(GPX_FOLDER):
        os.mkdir(GPX_FOLDER)
    s = requests.Session()
    ...
    for run in runs:
        ...
        try:
            ...
            track = parse_raw_data_to_nametuple(
                run_data, old_gpx_ids, with_download_tcx
            )
            tracks.append(track)
...
def parse_points_to_tcx(run_points_data, start_time):
    ...
    return create_tcx_file(points_dict_list, 'track_id')  # replace 'track_id' with your actual track id

def create_tcx_file(points, track_id):
    """Create a TCX file from the given points."""
    tcx = etree.Element("TrainingCenterDatabase", xmlns="http://www.garmin.com/xmlschemas/TrainingCenterDatabase/v2")
    activities = etree.SubElement(tcx, "Activities")
    activity = etree.SubElement(activities, "Activity", Sport="Running")
    lap = etree.SubElement(activity, "Lap", StartTime=points[0]['time'].isoformat())
    track = etree.SubElement(lap, "Track")

    for point in points:
        trackpoint = etree.SubElement(track, "Trackpoint")
        etree.SubElement(trackpoint, "Time").text = point['time'].isoformat()
        ...
        etree.SubElement(trackpoint, "AltitudeMeters").text = str(point.get('elevation', ''))
        etree.SubElement(trackpoint, "HeartRateBpm").text = str(point.get('heart_rate', ''))  # replace with your actual heart rate

    tree = etree.ElementTree(tcx)
    tree.write(f"{track_id}.tcx", pretty_print=True)

def download_keep_tcx(tcx_data, keep_id):
    try:
        print(f"downloading keep_id {str(keep_id)} tcx")
        file_path = os.path.join(GPX_FOLDER, str(keep_id) + ".tcx")
...
def run_keep_sync(email, password, with_download_tcx=False):
    generator = Generator(SQL_FILE)
    old_tracks_ids = generator.get_old_tracks_ids()
    new_tracks = get_all_keep_tracks(email, password, old_tracks_ids, with_download_tcx)
...
if __name__ == "__main__":
    ...
    run_keep_sync(options.phone_number, options.password, options.with_tcx)
